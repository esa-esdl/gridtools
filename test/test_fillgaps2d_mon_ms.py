...
class Resample2dWithMeanOfNearestMultiScaleTest(unittest.TestCase):
    def _test_fillgaps(self, src, desired_out, desired_gap_count):
        src = np.array(src)
        actual_out, actual_gap_count = gtg.fillgaps2d(src, method=gtg.GF_MEAN_OF_NEAREST_MULTI_SCALE)
        assert_almost_equal(actual_out, np.array(desired_out))
        self.assertEqual(actual_gap_count, desired_gap_count)
