...
def fwt97(s, width, height):
    ...
    for col in range(width):  # Do the 1D transform on all cols:
        ...
        for row in range(1, height - 1, 2):
            s[row, col] += WT97_A1 * (s[row - 1, col] + s[row + 1, col])
        s[height - 1, col] += 2 * WT97_A1 * s[height - 2, col]  # Symmetric extension

        # Update 1. y0
        for row in range(2, height, 2):
            s[row, col] += WT97_A2 * (s[row - 1, col] + s[row + 1, col])
        s[0, col] += 2 * WT97_A2 * s[1, col]  # Symmetric extension

        # Predict 2.
        for row in range(1, height - 1, 2):
            s[row, col] += WT97_A3 * (s[row - 1, col] + s[row + 1, col])
        s[height - 1, col] += 2 * WT97_A3 * s[height - 2, col]

        # Update 2.
        for row in range(2, height, 2):
            s[row, col] += WT97_A4 * (s[row - 1, col] + s[row + 1, col])
        s[0, col] += 2 * WT97_A4 * s[1, col]
